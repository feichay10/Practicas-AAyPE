...
dim3 blocksPerGrid1( (M + threadsPerBlock1.x - 1) / threadsPerBlock1.x , (N +
threadsPerBlock1.y - 1) / threadsPerBlock1.y ); MxN hilos distribuidos en
bloques de (p x q) hilos, cada hilo encargado de calcular un elemento de la
matriz C
*/

__global__ void matMul1(float *d_A, float *d_B, float *d_C, unsigned d_fil,
                        unsigned d_inner, unsigned d_col, size_t pitchA,
                        size_t pitchB, size_t pitchC) {
...
dim3 blocksPerGrid1x( (M + threadsPerBlock1x.x - 1) / threadsPerBlock1x.x , (N +
threadsPerBlock1x.y - 1) / threadsPerBlock1x.y ); MxN hilos distribuidos en
bloques de (p x q) hilos, con punteros, cada uno encargado de un elemento de la
matriz C
*/

__global__ void matMul1x(float *d_A, float *d_B, float *d_C, unsigned d_fil,
                         unsigned d_inner, unsigned d_col, size_t pitchA,
                         size_t pitchB, size_t pitchC) {
  ...
  if ((id_x < d_col) && (id_y < d_fil)) {
    ...
    for (int k = 0; k < d_inner; k++) {
      acc = acc + (*elema++) * (*elemb);
      elemb = (float *)((char *)elemb +
                        pitchB);  // Avanza el puntero por la columna de B
    }
    *elemc = acc;
  }
}
...
/*
    ...
    se lanzan NxM bloques de I hilos, cada hilo hace su producto de dos
   elementos y lo suma de forma atómica a una variable compartida, el hilo 0 la
   escribe en C
*/

__global__ void matMul2(float *d_A, float *d_B, float *d_C, unsigned d_fil,
                        unsigned d_inner, unsigned d_col, size_t pitchA,
                        size_t pitchB, size_t pitchC) {
...
__global__ void matMul3(float *d_A, float *d_B, float *d_C, unsigned d_fil,
                        unsigned d_inner, unsigned d_col, size_t pitchA,
                        size_t pitchB, size_t pitchC) {
  // NxM bloques de I hilos
  unsigned i = blockIdx.x;
  ...
  float *el_c =
      (float *)((char *)d_C + i * pitchC) + j;  // puntero a elemento a escribir
  if (k == 0) {
    *el_c = 0.0;
  }
...
/*
    ...
    Se lanzan M bloques de I hilos (un bloque por columna de B), colaboran cada
   uno leyendo un elemento de la columna de B, y van multiplicando el elemento
   por el correspondiente de A sec. y suman atomic en var compartida
*/

__global__ void matMul4(float *d_A, float *d_B, float *d_C, unsigned d_fil,
                        unsigned d_inner, unsigned d_col, size_t pitchA,
                        size_t pitchB, size_t pitchC) {
  ...
  float el_b = *((float *)((char *)d_B + k * pitchB) +
                 j);  // Obtiene el elemento de B asociado a este hilo
  for (int i = 0; i < N; i++) {
    if (k == 0) {
      acc = 0.0;
    }
...
/*
    ...
    Se lanzan M bloques de I hilos (un bloque por columna de B), colaboran cada
   uno leyendo un elemento de la columna de B, y van multiplicando el elemento
   por el correspondiente de A sec. y suman atomic elem final de C
*/

__global__ void matMul4b(float *d_A, float *d_B, float *d_C, unsigned d_fil,
                         unsigned d_inner, unsigned d_col, size_t pitchA,
                         size_t pitchB, size_t pitchC) {
  // M bloques de I hilos (columnas de B)
  unsigned j = blockIdx.x;
...
/*
    ...
    Se lanzan N bloques de I hilos (un bloque por fila de A), colaboran cada uno
   leyendo un elemento de la fila de A, y van multiplicando el elemento por el
   correspondiente de B sec. y suman atomic en var compartida
*/

__global__ void matMul5(float *d_A, float *d_B, float *d_C, unsigned d_fil,
                        unsigned d_inner, unsigned d_col, size_t pitchA,
                        size_t pitchB, size_t pitchC) {
  ...
  float el_a = *((float *)((char *)d_A + i * pitchA) +
                 k);  // Obtiene el elemento de A asociado a este hilo
...
/*
    ...
    Se lanzan N bloques de I hilos, colaboran cada uno leyendo un elemento de la
   fila de A, y van multiplicando el elemento por el correspondiente de B sec. y
   suman atomic en elemento de C corresp.
*/

__global__ void matMul5b(float *d_A, float *d_B, float *d_C, unsigned d_fil,
                         unsigned d_inner, unsigned d_col, size_t pitchA,
                         size_t pitchB, size_t pitchC) {
  // N bloques de I hilos (filas de A)
  unsigned i = blockIdx.x;
  ...
  for (int j = 0; j < M; j++) {  // Iteramos por las M columnas de B
    ...
    __syncthreads();  // espera a que se haya inicializado
    atomicAdd(&rowc[j],
              el_a * rowb[j]);  // Todos los hilos escriben secuencialmente
  }
}
...
/*
    ...
    Se lanzan M x N bloques de (BSIZE x BSIZE), cada bloque es responsable del
   calculo de submatriz C de tamaño (BSIZE x BSIZE) multiplicando cada elemento
   de una macrofila de A compuesta por I/BSIZE submatrices por cada elemento de
   una macrocolumna de B compuesta por I/BSIZE submatrices. Cada uno de estos
   productos de submatrices se acumula en la submatriz de C. Cada una de las dos
   submatrices a multiplicar se copia a memoria compartida y cada hilo del
   bloque realiza el cálculo parcial de un elemento de la submatriz C y lo suma
   en ella
*/
__global__ void matMul6(float *d_A, float *d_B, float *d_C, unsigned d_fil,
                        unsigned d_inner, unsigned d_col, size_t pitchA,
                        size_t pitchB, size_t pitchC) {
  ...
  for (int m = 0; m < blockInner; m++) {
    ...
    As[row][col] =
        (colA < I)
            ? *((float *)((char *)d_A + (bRow * BSIZE + row) * pitchA) + colA)
            : 0.0;
    Bs[row][col] =
        (rowB < I)
            ? *((float *)((char *)d_B + rowB * pitchB) + bCol * BSIZE + col)
            : 0.0;
    __syncthreads();  // Esperar a acabar la copia local de Asub y Bsub
    // Multiplicar Asub and Bsub
    for (int e = 0; e < BSIZE; e++)
      if (elemC != NULL) *elemC += As[row][e] * Bs[e][col];
    __syncthreads();  // acabar la acumulación en Csub del producto antes de
                      // pasar al siguiente
  }
}
...
int cmpMat(float *d_C, size_t pitchC, float *h_C, float *h_R, float tol) {
  CUDA_CHECK_RETURN(cudaMemcpy2D(h_C, M * sizeof(float), d_C, pitchC,
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < M; j++) {
      if (fabs(h_C[(i * M) + j] - h_R[(i * M) + j]) > tol) {
        fprintf(
            stderr,
            "Result verification failed at element (%d, %d)! GPU=%f, CPU=%f\n",
...
int main(void) {
  ...
  // printf("[Pitch of A is %d, Width is %d, Width in bytes is %d and height is
  ...
  // Copy the host input matrices A and B in host memory to the device input
  // matrices in device memory
  CUDA_CHECK_RETURN(cudaMemcpy2D(d_A, pitchA, h_A, I * sizeof(float),
                                 I * sizeof(float), N, cudaMemcpyHostToDevice));
  CUDA_CHECK_RETURN(cudaMemcpy2D(d_B, pitchB, h_B, M * sizeof(float),
                                 M * sizeof(float), I, cudaMemcpyHostToDevice));

  // Launch the Matrix product CUDA Kernels
  dim3 threadsPerBlock1(32, 32);
  dim3 blocksPerGrid1((M + threadsPerBlock1.x - 1) / threadsPerBlock1.x,
                      (N + threadsPerBlock1.y - 1) / threadsPerBlock1.y);
  printf("CUDA kernel 1 launch with (%d, %d) blocks of (%d, %d) threads\n",
         blocksPerGrid1.x, blocksPerGrid1.y, threadsPerBlock1.x,
         threadsPerBlock1.y);
  startTimer(&start, &stop);
  matMul1<<<blocksPerGrid1, threadsPerBlock1>>>(d_A, d_B, d_C, N, I, M, pitchA,
                                                pitchB, pitchC);
  ...
  dim3 blocksPerGrid1x((M + threadsPerBlock1x.x - 1) / threadsPerBlock1x.x,
                       (N + threadsPerBlock1x.y - 1) / threadsPerBlock1x.y);
  printf("CUDA kernel 1x launch with (%d, %d) blocks of (%d, %d) threads\n",
         blocksPerGrid1x.x, blocksPerGrid1x.y, threadsPerBlock1x.x,
         threadsPerBlock1x.y);
  startTimer(&start, &stop);
  matMul1x<<<blocksPerGrid1x, threadsPerBlock1x>>>(d_A, d_B, d_C, N, I, M,
  ...
  startTimer(&start, &stop);
  matMul2<<<blocksPerGrid2, threadsPerBlock2>>>(d_A, d_B, d_C, N, I, M, pitchA,
                                                pitchB, pitchC);
  CUDA_CHECK_RETURN(cudaGetLastError());
  stopAndPrintTimer(&start, &stop);
  if (cmpMat(d_C, pitchC, h_C, h_R, 1.0e-3)) printf("Test PASSED\n");
  ...
  startTimer(&start, &stop);
  matMul3<<<blocksPerGrid3, threadsPerBlock3>>>(d_A, d_B, d_C, N, I, M, pitchA,
                                                pitchB, pitchC);
  CUDA_CHECK_RETURN(cudaGetLastError());
  stopAndPrintTimer(&start, &stop);
  if (cmpMat(d_C, pitchC, h_C, h_R, 1.0e-3)) printf("Test PASSED\n");
  ...
  printf("CUDA kernel 4 launch with %d blocks of %d threads\n", blocksPerGrid4,
         threadsPerBlock4);
  startTimer(&start, &stop);
  matMul4<<<blocksPerGrid4, threadsPerBlock4>>>(d_A, d_B, d_C, N, I, M, pitchA,
                                                pitchB, pitchC);
  CUDA_CHECK_RETURN(cudaGetLastError());
  stopAndPrintTimer(&start, &stop);
  if (cmpMat(d_C, pitchC, h_C, h_R, 1.0e-3)) printf("Test PASSED\n");
  ...
  startTimer(&start, &stop);
  matMul4b<<<blocksPerGrid4b, threadsPerBlock4b>>>(d_A, d_B, d_C, N, I, M,
                                                   pitchA, pitchB, pitchC);
  CUDA_CHECK_RETURN(cudaGetLastError());
  stopAndPrintTimer(&start, &stop);
  if (cmpMat(d_C, pitchC, h_C, h_R, 1.0e-3)) printf("Test PASSED\n");
  ...
  printf("CUDA kernel 5 launch with %d blocks of %d threads\n", blocksPerGrid5,
         threadsPerBlock5);
  startTimer(&start, &stop);
  matMul5<<<blocksPerGrid5, threadsPerBlock5>>>(d_A, d_B, d_C, N, I, M, pitchA,
                                                pitchB, pitchC);
  CUDA_CHECK_RETURN(cudaGetLastError());
  stopAndPrintTimer(&start, &stop);
  if (cmpMat(d_C, pitchC, h_C, h_R, 1.0e-3)) printf("Test PASSED\n");
  ...
  startTimer(&start, &stop);
  matMul5b<<<blocksPerGrid5b, threadsPerBlock5b>>>(d_A, d_B, d_C, N, I, M,
                                                   pitchA, pitchB, pitchC);
  CUDA_CHECK_RETURN(cudaGetLastError());
  stopAndPrintTimer(&start, &stop);
  if (cmpMat(d_C, pitchC, h_C, h_R, 1.0e-3)) printf("Test PASSED\n");
  ...
  printf("CUDA kernel 6 launch with (%d, %d) blocks of (%d, %d) threads\n",
         blocksPerGrid6.x, blocksPerGrid6.y, threadsPerBlock6.x,
         threadsPerBlock6.y);
  startTimer(&start, &stop);
  matMul6<<<blocksPerGrid6, threadsPerBlock6>>>(d_A, d_B, d_C, N, I, M, pitchA,
                                                pitchB, pitchC);
  CUDA_CHECK_RETURN(cudaGetLastError());
  stopAndPrintTimer(&start, &stop);
  if (cmpMat(d_C, pitchC, h_C, h_R, 1.0e-3)) printf("Test PASSED\n");
